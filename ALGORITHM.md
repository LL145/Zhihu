...
flowchart TD
    ...
    C2 --> ROUTE
    C3 --> ROUTE
    ROUTE -- "问具体事" --> E1["主断＝问语卦：体用生克（tiyong.py）<br/>互变论中终、卦气论旺衰、爻辞为参<br/>meihua:2:tiyong · meihua:2:zhan:* · shuogua:11"]
    ROUTE -- "问命格/时运（有盘）" --> Z1["主断＝紫微：命宫主星 / 大限入限诀<br/>ziwei:2:ming:* · ziwei:3:daxian"]
    E1 --> V["定例断辞（卦：总诀明文映射；盘：庙陷／大限论明文）"]
    Z1 --> V
...
| 7 | 体用生克与选文（卦侧） | 动爻所在之卦为用、不动为体，八宫五行论生克（`tiyong.py` 机断）：**用卦与体之关系为主断**——用生体／体克用／比和／体生用／用克体五者各取总诀明文；互卦（去初上取中四爻）为事之中间、变卦（之卦所变之一卦）为事之终，各与体论生克；卦气依问时农历月令论体卦旺衰；体党用党计其数；生体／克体之卦各附总诀「某卦生体／克体」具象句；所问占章（类别映射→题材回落→人事占通例）中对应关系之句并为主断依据，占章论应期者附生克之卦「时序」（万物属类）为参。动爻爻辞降为参（易辞；从卷一占例「易辞不吉矣……以卦论之」之序），本卦、之卦卦辞为参，本卦并附序卦（卦之由来）、杂卦（卦之性）逐卦一行；体用取象入解读（说卦广象＋《梅花》万物属类逐卦附于体用之下）。**约定**：辰戌丑未月依「四季之月」条论旺衰、余月依四时（底本春条与四季条于辰月重叠，取四季条）；乾坤纯卦依「乾坤无互，互其变卦」；占章之句按关系字样机械截取，无对应字样不取；序卦杂卦切片运行时自整篇派生（序卦按「受之以X」，杂卦按分句句首卦名），乾坤咸无序卦句、旅不居杂卦句首则不切 | `meihua:2:tiyong`（体用总诀：「体克用，诸事吉；用克体，诸事凶。体生用，有耗失之患；用生体，有进益之喜。体用比和，则百事顺遂」「用为事之端……互为事之中间……变为事之终」「体党多而体势盛」、逐卦生体克体之象）、`meihua:1:bagong`（八宫五行）、`meihua:1:wuxing`（生克）、`meihua:1:hugua`（互卦起例，乾坤无互）、`meihua:1:guaqi:wang`／`shuai`（卦气旺衰）、`meihua:2:zhan:*`（十八占；人事占为通例，「占者以类而推之可也」）、`meihua:1:xiang:wanwu:*`（时序）、`xugua:{卦}:gua`／`zagua:{卦}:gua`、`shuogua:11`（广象）＋`shuogua:7`（性情） |
...
| 9 | 定例断辞 | **卦侧（梅花法）**：用卦与体之关系按总诀明文直接映射——用生体、体克用、比和为吉，体生用为谨，用克体为凶（标「总诀明文映射」，不经字样提取）；卦气衰旺、互变生克不改此单一定例，只入解读之参（异断须明据）。**朱子法**（留存）：主断经文断辞字样（吉凶悔吝厉无咎…）按明示优先级机取归类；审定记两处（audited 如实标注）：订正个案按 cite_id 覆盖（verdict_overrides.json），审定确认名单（verdict_audit.json）内机取维持而 audited 置真。两法之定例各附断辞之义（系辞「吉凶者，言乎其失得也；悔吝者，言乎其小疵也；无咎者，善补过也」） | 总诀明文（`meihua:2:tiyong`）；断辞字样即经文本文；提取优先级为约定（verdict.py 表）；断辞之义出 `xici:shang:3`；审定轮次与方法录 data/PROOFREADING.md |
| 10 | 语境合参 | 非主断的占法只作"人的语境"入解读：时间卦论当下之势（两种主断下皆然），姓名卦论问者之位，紫微盘论秉性禀赋（问事时），卦为主断时另设占例块：按本卦体用生克字样（生体／克体／比和，先用后互变）自《梅花易数》卷一占例池机械召回同类之例（至多二则、依库序、命中数进凭证），可引其法为比、不得以例中所断为本卦之断；盘为主断问命格时加取三方四正（财帛、官禄、迁移）诸星断语为参，西洋本命盘作异邦占法之参照（命理主断时；语境块附盘面事实＋《占星四书》总纲章 1:5/1:16/1:20/1:22 恒附、论题章按判类与题材附——命格 3:18、时运 4:10、题材经分宫映射取卷三卷四对应章；有出生地并附上升中天分府事实与 3:4（轴续倾强弱）、3:3（生时难准之义）两章；寿夭疾病生死诸章属红线主题不附，藏书仍可检索）；均不出第二吉凶。语境原文可引不可断：卦侧语境备经（爻辞、卦辞）、传（小象、彖）、注（动爻王弼注）各一体，西洋本命盘备盘面事实与《占星四书》章句，解读宜择要参引为佐（旁征博引）——提示语明言命理主断附西洋盘时宜在理由中至少参及一次其盘面事实（七曜落宫、得位、相位、上升分府，转述即可、不受引文校验；引《占星四书》原文则须逐字英文），存疑标注之项勿参。盘为主断时另设书桌块：按主断宫主星星名——问涉具体题材时并以所涉之宫名（问事类别经分宫映射归于宫名）——自《卷一》赋文格诀池机械召回候选断语，召回规则与命中数进凭证，池内择引、不合盘面者勿引 | 约定（DESIGN §6.2 之推广）；解读层硬性规则＋校验器共同保证 |
...
- 西洋本命盘（命理主断时）：同为生辰确定之盘，以异邦占法作参照
  ...
  中译只作转述；不得据以立断或另出吉凶；
- 占例（确定性召回层，卦为主断时）：按体用生克字样自卷一占例池机械
  召回同类之例入语境，召回规则与命中数进凭证；占例是邵子断法之范，
  可引其法为比，不得以例中所断之事为本卦之断；
- 校验器主断必据：断语至少须标注一处主断明文（梅花法即体用总诀与
  所问占章；朱子法即主断经文）——爻辞、卦辞可并引，不得单独立断，
  结构上保证吉凶自体用出；
...
【理由】为主断明文（梅花法为总诀之句并占章之句，朱子法为主断经文）
及断辞之义（系辞），格式不变；一览并列体用生克一行，凭证另立
「体用生克凭证」节（体用互变生克、卦气、党、生克体之卦、占例召回）。
