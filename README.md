...
有典可依、可复现的双引擎占断：**卦断事，盘论人**。设计文档见 [DESIGN.md](DESIGN.md)。

**核心原则：LLM 不算卦也不算命，只解释。** 起卦、排盘、选文、结论全部是
确定性代码；大模型只负责把规则选定的《周易》《紫微斗数全书》原文讲清楚，
其解读中的每条引文都经逐字校验后方可输出，校验不过即拒绝（降级为仅原文与结论）。
...
tests/           162 项测试：起卦确定性、占法七情形、断辞映射、问事分类、
