...
- **正统**——流程与文本俱出典籍明文：朱子《易学启蒙》占法、《梅花易数》
  起数与体用占诀、《卜筮正宗》铜钱法、《紫微斗数全书》安星诀与断语，
  另收《说卦传》《文言》为经传取象与释义之资；解读中每条引文
  逐字校验，校验不过即拒绝输出；
...
十二宫），类别只定选文框架；问题原文仍逐字入铜钱种子、问事分宫与占者。

输出第二行即标明本次所用引擎（「引擎：易经事引擎（卦断事）」或
「引擎：紫微命引擎（盘论人）」），此后包含：问事类别（判类三级，决定
解读落点）→
卦象（本卦、之卦、互卦、动爻）→ 所据经文（占法规则选定的原文，
含彖、象传、**王弼注**与乾坤两卦之**《文言》**，逐条标出处；梅花法
另按体用附**《说卦传》广象**与《梅花易数》**体用总诀**、所问类**占章**）→
**定例断辞**（吉/凶/谨/忌…，
自经文断辞字样机取，作对照基准）→ **占断与讲释**（占者：大模型——
衡所据经文而下占断，句末标出处，异于定例须明据，注家之言与取象
不得单独立断；引文逐字校验）→
...
**卦盘并占（两问两断）**：命理之问（命格/时运）附生辰时，可另开
「卦盘并占」（界面勾选或 `--both`）——盘为主照常命断之外，再以同一
问题、同一时刻起卦作一份完整卦断（占章按题材自动附取，如问财附
求财占）。两份**各有定例对照、各自占断、各自存证**，两次解读各在
各的文本池内生成、互不见对方原文——**并陈，不合断**：命卜异术，
问的本是两件事（盘断其势，卦断其事），古法兼请不为渎，但无定例
可合两断为一，故程序也不合。追问两侧并答。
...
yijing_agent/
  ...
  service.py     双引擎流水线门面（路由/会话/解读/追问/卦盘并占，GUI 复用）
