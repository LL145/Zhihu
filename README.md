...
**问随便问，类按典分**：问事分类是三级判类——关键词规则先行（确定性）；
规则未中且已配模型时由占者判类（输出标注〔占者判类〕）；界面类别下拉
（默认「自动判类」）或 `--topic 情感` 可手动指定（标注〔用户指定〕，最
优先）。定类而占本是古法之绪（《梅花易数》十八占、六爻按类取用神、紫微
十二宫），类别只定选文框架；问题原文仍逐字入问事分宫与占者。
...
tests/           253 项测试：起卦确定性（含问语起卦）、占法七情形、
                 断辞映射、问事分类、
