...
【断语】传统断辞句（句末标 [cite_id]，无据不断，异于定例须明据）→
【理由】引古籍原文（逐字校验）并解释 → 【建议】→ 卦盘一览（各占法
一两行：主断/参/语境的分工与定例断辞）→ 所据原文节选（主断附全文，
其余列编号，`corpus --cite` 可随时取全文；含彖象传、**王弼注**、乾坤
《文言》，梅花法另按体用附**《说卦传》广象**、**体用总诀**与所问类
**占章**）→ **占断存证**（模型名与输出 SHA-256）→ 起卦/排盘凭证
（取数与算式，任何人可复算）。
...
tests/           205 项测试：起卦确定性、占法七情形、断辞映射、问事分类、
