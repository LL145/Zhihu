...
**单一模式，三占同起**（v3.2，ALGORITHM.md）：输入固定五项——问什么＋
姓名＋生日＋出生时辰＋性别。时间卦（梅花·年月日时起例）、姓名卦
（梅花·字占，论问者之位）、紫微命盘（论秉性禀赋与限运）同时起，全部
确定性、无随机数；**吉凶只从主断一处出**（问具体事→时间卦主断，问命格
/时运→紫微主断），其余占法只作"人的语境"入解读——结构上杜绝自相矛盾。
